...
const Dashboard = ({ onAddClick, onEditClick ,onViewClick, onCatClick }) => {
  ...
  useEffect(() => {
    const fetchCategories = () => {
      const baseCategories = ["All Bricks", "Hamptons", "Classic Reds", "Multies", "Rumbled", "Yellows", "Darks", "Reclaimed"];
      ...
      <div className="relative max-w-full mx-auto p-2 lg:p-2">
        ...
        <div className={viewMode === 'grid' ? "grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6" : "space-y-6"}>
          ...
          {filteredProducts.length === 0 && (
            <div className="col-span-full py-20 text-center">
              ...
              <p className="text-stone-400 font-bold uppercase tracking-widest text-sm">No items found</p>
            </div>
          )}
        </div>
        ...
        {totalPages > 1 && (
          <div className="flex justify-center items-center gap-3 mt-12 pb-10">
            <button disabled={currentPage === 1} onClick={() => setCurrentPage(prev => prev - 1)} className="p-3 rounded-xl border border-stone-200 bg-white disabled:opacity-30 hover:bg-orange-50 transition-colors">
              <ChevronLeft size={20} />
            </button>
            <div className="flex gap-2">
              {[...Array(totalPages)].map((_, index) => (
                <button key={index + 1} onClick={() => setCurrentPage(index + 1)} className={`w-10 h-10 rounded-xl font-bold transition-all border ${currentPage === index + 1 ? "bg-orange-600 text-white border-orange-600 shadow-lg shadow-orange-100" : "bg-white text-stone-600 border-stone-200 hover:border-orange-600"}`}>
                  {index + 1}
                </button>
              ))}
            </div>
            <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(prev => prev + 1)} className="p-3 rounded-xl border border-stone-200 bg-white disabled:opacity-30 hover:bg-orange-50 transition-colors">
