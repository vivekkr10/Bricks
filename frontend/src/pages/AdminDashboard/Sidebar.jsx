...
const Sidebar = ({ activePage, setActivePage, handleLogout }) => {
  ...
  return (
    <motion.div 
      ...
      // Updated Background to Stone-900 (#1C1917) and Border to Stone-800
      className="h-screen bg-[#1C1917] text-[#F5F5F4] flex flex-col fixed left-0 top-0 shadow-2xl z-50 border-r border-[#292524] transition-all duration-300"
    >
      ...
      <div 
        ...
        onClick={() => setIsCollapsed(!isCollapsed)} // Made the header clickable for easier toggle
      >
        
        {/* LOGO AREA */}
        <div className="flex items-center gap-3">
          {/* Logo Box uses the Accent Orange */}
      ...
      <div className="px-4">
        {/* Divider using Stone-800 */}
        <div className="h-[1px] bg-[#292524]" />
      </div>
      ...
      <div className="p-4 mt-auto border-t border-[#292524]">
        ...
        >
          <div className="p-2 bg-[#292524] rounded-lg group-hover:bg-[#EA580C] text-[#78716C] group-hover:text-white transition-colors duration-300 shrink-0">
            <LogOut size={20} />
          </div>
          {!isCollapsed && (
            <span className="font-bold text-sm text-[#78716C] uppercase tracking-widest group-hover:text-white whitespace-nowrap">Logout</span>
          )}
        </button>
      </div>
    </motion.div>
...
const NavItem = ({ icon, label, active, isCollapsed, onClick }) => (
  <div 
    ...
    className={`flex items-center gap-4 p-3 rounded-xl cursor-pointer transition-all duration-300 group relative border-l-4 ${
      ...
      : 'hover:bg-[#292524]/50 border-transparent text-[#78716C] hover:text-[#F5F5F4]'
