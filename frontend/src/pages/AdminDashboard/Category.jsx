...
import { Plus, Trash2, ImageIcon, ArrowLeft, Upload, CheckCircle, ChevronLeft, ChevronRight } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-white rounded-[2rem] p-4 md:p-8 font-sans">
      <div className="max-w-4xl mx-auto">
        ...
        <h1 className="text-3xl font-black text-[#1C1917] mb-4">Manage <span className="text-orange-600">Categories</span></h1>

        {/* Form Card */}
        <div className="bg-white rounded-[2rem] border border-stone-200 p-6 md:p-8 shadow-xl mb-12">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-4">
              <label className="block text-xs font-black uppercase tracking-widest text-stone-400">Category Thumbnail</label>
              <div className={`relative h-56 rounded-[1.5rem] border-2 border-dashed flex flex-col items-center justify-center transition-all overflow-hidden ${image ? "border-orange-500" : "border-stone-200 hover:border-orange-300 bg-stone-50"}`}>
                {image ? (
                  <>
                    <img src={image} alt="Preview" className="w-full h-full object-cover" />
                    <button onClick={() => setImage(null)} className="absolute top-2 right-2 bg-red-500 text-white p-1.5 rounded-full shadow-lg"><Trash2 size={16} /></button>
                  </>
                ) : (
                  <label className="cursor-pointer flex flex-col items-center group">
                    <div className="p-4 bg-white rounded-full shadow-md group-hover:scale-110 transition-transform"><Upload className="text-orange-600" size={24} /></div>
                    <span className="mt-3 text-sm font-bold text-stone-500">Click to upload photo</span>
        ...
        <div className="space-y-6 relative">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-black text-[#1C1917] flex items-center gap-2">
                All Categories <span className="bg-stone-200 text-stone-600 text-xs px-2 py-0.5 rounded-full">{categories.length}</span>
            </h2>
            
            {/* Show controls only if more than 4 items */}
            {categories.length > 4 && (
              <div className="flex gap-2">
                <button onClick={() => scroll("left")} className="p-2 bg-white border border-stone-200 rounded-full hover:bg-orange-600 hover:text-white transition-all shadow-sm">
                  <ChevronLeft size={20} />
                </button>
                <button onClick={() => scroll("right")} className="p-2 bg-white border border-stone-200 rounded-full hover:bg-orange-600 hover:text-white transition-all shadow-sm">
